<?php 
    ...
    $connection= mysqli_connect('localhost','root','','vehicle1');
    ...
    // $query1 = "SELECT booking.`booking_id`, booking.`req_date` ,booking.`ret_date`, booking.`destination`, booking.`veh_reg`, booking.`driverid`, tripcost.`total_km` ,tripcost.`oil_cost`, tripcost.`extra_cost`,tripcost.`total_cost`,tripcost.`paid` FROM `booking` , `tripcost` WHERE booking.username=tripcost.username and booking.username=`$username` AND booking.booking_id=tripcost.booking_id;";
    //echo $query;
